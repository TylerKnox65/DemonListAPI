         ...
         while True:             
            ...
            if event == None:
                ...
                if event in "load":
                    ...
                    self.window.size = ((self.window_size[0] + 50),self.window_size[1] + 50)
                    self.window.size = ((self.window_size[0]),self.window_size[1])

    def convertimage(self,url):
        sg.one_line_progress_meter("Loading",self.num,1000,key="real") #Literally the worst loading window...AGAIN, being replaced by loadingscreen.py when i finish
        jpg_data = (
